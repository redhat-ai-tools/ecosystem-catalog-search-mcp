...
if ENABLE_METRICS and PROMETHEUS_AVAILABLE:
    ...
    search_requests_total = Counter(
        ...
        'Total number of Red Hat search API requests',
        ['status']
    )
    
    search_request_duration_seconds = Histogram(
        'mcp_search_request_duration_seconds',
        'Duration of Red Hat search API requests in seconds'
    )
...
def track_search_request(start_time: float, success: bool) -> None:
    """Track Red Hat search API request metrics"""
    if ENABLE_METRICS and PROMETHEUS_AVAILABLE:
        status = 'success' if success else 'error'
        search_requests_total.labels(status=status).inc()
        
        duration = time.time() - start_time
        search_request_duration_seconds.observe(duration)
